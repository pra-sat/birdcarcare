...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  🔴 ข้อยกเว้น (14 ก.ย. 2569) — แอดมินกดปุ่ม "ดูข้อมูลร้าน" มาเอง
    //
    //  หน้าแอดมินมีปุ่มให้มาดูหน้านี้ได้ แต่ถ้าไม่กันไว้ พอมาถึงปุ๊บ
    //  โค้ดข้างล่างจะเด้งกลับหน้าแอดมินทันที = กดปุ่มแล้วเหมือนไม่มีอะไรเกิดขึ้น
    //  ปุ่มนั้นจึงส่ง ?stay=1 มาด้วย แปลว่า "ฉันตั้งใจมาดูหน้านี้ อย่าเด้ง"
    //
    //  ไม่กระทบความปลอดภัย เพราะการ "ไม่เด้ง" คือการเห็นหน้าลูกค้าธรรมดา
    //  ซึ่งใครก็เห็นได้อยู่แล้ว ไม่ใช่การให้สิทธิ์อะไรเพิ่ม
    const stayHere = /[?&]stay=1\b/.test(window.location.search);
    ...
    if (!stayHere && readAdminCache(userId)) {
